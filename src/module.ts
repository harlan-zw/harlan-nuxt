...
export async function generateRegistryTypesTemplate(options: ModuleOptions, rootDir: string, templateDir: string): Promise<string> {
  const files = await resolveJobFiles(options, rootDir)
  assertUniqueGeneratedJobNames(files, options, rootDir)
  const loaderTypeLines = files.map((file) => {
    return `  ${JSON.stringify(toJobName(file, options, rootDir))}: () => Promise<typeof import(${JSON.stringify(toImportPath(templateDir, file).replace(/\.ts$/, ''))})['default']>,`
  })
  return [
    '/* This file is generated by nuxt-cf-jobs. Do not edit directly. */',
    `import type { JobDefinitionsByNameOfLoaders, JobMessageByName, JobMessageByQueue, JobNameOf, JobPayloadByName, JobPayloadOf, JobQueueByName, QueueNameOf, QueueConsumerOptions } from '#cf-jobs/server'`,
    `export type { QueueConsumerOptions }`,
    '',
    'export declare const jobLoaders: {',
    ...loaderTypeLines,
    '}',
    'export declare const jobs: readonly unknown[]',
    'export declare const app: ReturnType<typeof import(\'#cf-jobs/server\').createCfJobsApp>',
