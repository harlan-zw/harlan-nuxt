...
export function installEventRegistryTemplates(options: ModuleOptions, nuxt: Nuxt, templateDir: string): void {
  ...
  nuxt.options.alias['#event-listeners/server'] = serverTemplate.dst
  const nitro = ((nuxt.options as unknown as { nitro?: Record<string, any> }).nitro ??= {})
  nitro.alias ||= {}
  nitro.alias['#event-listeners/server'] = serverTemplate.dst
...
export function renderEventRegistry(plan: EventRegistryBuildPlan): string {
  ...
  return [
    ...
    `import { createGeneratedEventRegistry, createGeneratedEventRuntime } from '@nuxtseo/event-listeners/server'`,
    `import { observeEventListener, observeEventListenerFallback } from '#event-listeners/observer'`,
    '',
...
export function renderEventRegistryTypes(plan: EventRegistryBuildPlan): string {
  ...
  return [
    '/* Generated by @nuxtseo/event-listeners. Do not edit. */',
    `import type { CommitEventPlanResult, DispatchReport, EventCommitUnitOfWork, EventDispatchContext, EventPayloadOf, EventPlan, QueuedDeliveryContext, EventListenerEnvelope } from '@nuxtseo/event-listeners/server'`,
    '',
...
function assertValidPlan(
  ...
  if (queuedListeners.length > 0 && !hasQueuedDeliveryContext)
    throw new Error(`Queued listener delivery requires eventListeners.queuedDeliveryContext; found: ${queuedListeners.map(listener => listener.meta.name).join(', ')}`)
