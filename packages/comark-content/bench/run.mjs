...
const FONT_CACHE_PATH = 'node_modules/.cache/nuxt/.nuxt/cache/fonts'
...
async function main() {
  ...
  const packageFingerprint = variant === 'candidate' ? await candidateFingerprint() : undefined
  const previous = await readFile(resultPath, 'utf8').then(JSON.parse).catch(() => undefined)
  const retainedSamples = previous?.status === 'running' && previous?.environment?.node === process.version && previous?.environment?.pnpm === PNPM_VERSION && previous?.packageFingerprint === packageFingerprint
    ? previous.samples ?? []
    : []
  ...
  try {
    const siteFontCache = join(SITE_ROOT, FONT_CACHE_PATH)
    if ((await stat(siteFontCache).catch(() => undefined))?.isDirectory())
      await cp(siteFontCache, fontCacheRoot, { recursive: true })
    else
      await mkdir(fontCacheRoot, { recursive: true })
    ...
    if (variant === 'candidate') {
      const candidatePackage = JSON.parse(await readFile(join(PACKAGE_ROOT, 'package.json'), 'utf8'))
      ...
      const exception = `  - '@harlan-zw/comark-content@${candidatePackage.version}'\n`
      const trustedWorkspace = ['minimumReleaseAgeExclude', 'trustPolicyExclude'].reduce(
        (value, key) => value.replace(`${key}:\n`, `${key}:\n${exception}`),
        workspace,
      )
      await writeFile(workspacePath, trustedWorkspace)
    }
