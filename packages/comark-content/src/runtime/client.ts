...
type Fetch = <T>(path: string, options: { method: 'POST', body: QueryRequest }) => Promise<T>
...
export const queryCollectionNavigation = <TName extends CollectionName | string>(collection: TName, fields: string[] = []) => request<ContentNavigationItem[]>({
  _tag: 'Navigation',
  collection,
  fields,
})
