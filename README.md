...
After-commit flow uses `planEvent`, then `commitEventPlan`. The caller-supplied unit of work owns the database transaction. It must stage the adapter's unpublished D1 statements beside domain writes, return `rolled-back` with zero queue evidence on rollback, or return the adapter's exact staged-delivery receipt only after D1 resolves. The runtime sends the immutable original plan after commit. A failed send remains an unpublished durable row for recovery.
