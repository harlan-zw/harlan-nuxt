...
function renderRegistryTemplate(plan: RegistryBuildPlan): string {
  ...
  return [
    ...
    // Pull `useRuntimeConfig` from the framework's virtual `#imports` alias, NOT
    // from the bare `nitropack/runtime` specifier. This file is emitted into the
    // consumer's buildDir, where `nitropack` is a transitive dep that is only
    // physically resolvable if it happens to be hoisted/symlinked there — a
    // reinstall or peer-conflict (named catalog) can drop that symlink and break
    // the build with "Cannot find package 'nitropack'". `#imports` is a
    // build-time alias the framework always provides (resolving to nitro's
    // `useRuntimeConfig` server-side), so it never depends on physical linking.
