...
describe('cf-jobs broadcast protocol', () => {
  it('builds Laravel-style channels and durable-object topics', () => {
    expect(cfJobChannel('j1')).toBe('job:j1')
    expect(cfJobBatchChannel('b1')).toBe('batch:b1')
    expect(cfJobSiteChannel('s1')).toBe('site:s1')
    expect(cfJobUserChannel(7)).toBe('user:7')
...
describe('cf-jobs broadcast lifecycle adapters', () => {
  it('fans job lifecycle events out to job, queue, batch, site, and user channels', async () => {
    const { env, published } = createBroadcastEnv()
  ...
  it('publishes batch progress to batch and site channels', async () => {
    const { env, published } = createBroadcastEnv()
