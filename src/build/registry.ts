...
function renderRegistryTemplate(plan: RegistryBuildPlan): string {
  ...
  return [
    ...
    // build, AND raw Node (it's emitted into the consumer's buildDir and nitro
    // dev imports it as an external `file://` module). So it must import NOTHING
    // that only resolves in one of those: `#imports` is a build-only Nuxt virtual
    // (dies in raw Node), and `nitropack/runtime` eagerly pulls nitro's
