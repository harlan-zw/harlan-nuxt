...
function renderRegistryTemplate(plan: RegistryBuildPlan): string {
  ...
  return [
    ...
    `import { useJobRuntimeConfig } from '@harlanzw/nuxt-cf-jobs/runtime-config'`,
    `import { createGeneratedCfJobsApp } from '@harlanzw/nuxt-cf-jobs/app'`,
    '',
    'export const jobs = [',
    ...entryLines,
    ']',
    `/** @type {import('@harlanzw/nuxt-cf-jobs/server').CfJobsApp<${jobsType}>} */`,
...
function renderRegistryTypesTemplate(plan: RegistryBuildPlan, registryAlias: string): string {
  ...
  const moduleBlocks = aliases.flatMap((alias, index) => [
    ...
    `  export type { CfJobsDurableRuntimeOptions, QueueConsumerOptions } from '@harlanzw/nuxt-cf-jobs/server'`,
    '  type Jobs = readonly [',
    ...jobTypeLines,
    '  ]',
  ...
  return [
    '/* This file is generated by nuxt-cf-jobs. Do not edit directly. */',
    `import type { BroadcastEnvelopeOf, BroadcastMessageOf, JobBroadcastEnvelopeByName, JobBroadcastMessageByName, JobMessageByName, JobMessageByQueue, JobNameOf, JobPayloadOf, JobQueueByName, QueueNameOf } from '@harlanzw/nuxt-cf-jobs/server'`,
    '',
    ...moduleBlocks,
  ].join('\n')
}
