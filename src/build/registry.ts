...
function renderRegistryTemplate(plan: RegistryBuildPlan): string {
  ...
  return [
    ...
    // `useRuntimeConfig` must come from the real `nitropack/runtime` specifier,
    // NOT a virtual alias like `#imports`: this generated file is loaded by raw
    // Node (outside the vite/nitro bundle), and Node can only resolve actual
    // package specifiers. `nitropack` is a direct dependency of any nitro app,
    // so it resolves from the consumer's node_modules. (If it ever fails with
    // "Cannot find package 'nitropack'", the consumer's install dropped the
    // symlink — fix the install, e.g. `pnpm install --force`; do not switch to
    // a build-time alias here.)
