...
| Derived data | Compact collection metadata, one Comark AST asset per document, navigation, surroundings, search sections, table of contents, and sitemap entries | Raw Markdown, a whole-collection AST blob, and fields unused by the fleet |
| Navigation | Content-addressed immutable GET endpoints for navigation, surroundings, and precomputed heading search sections; navigation tree and hidden entries | Stable cache keys across changed content; navigation queries that decode document ASTs; general-purpose customization not present in scoped content |
| Rendering | Direct Comark nodes, semantic HTML, renderer classes, `ContentProseX` and `ProseX` HTML overrides, `ContentX`, `ProseX`, and plain component resolution, aliases, named slots, bound props, `unwrap="p"`, described images, styled links, and Rangi code highlighting. Generate static imports only for tags present in the ingested AST. Rangi defaults include GitHub light and dark themes plus `dotenv`, `env`, `robots`, `robots-txt`, and `robots.txt`. | MDC nodes, MDC parser, `@nuxtjs/mdc`, runtime Markdown parsing, dynamic component lookup, unused component imports, a general MDC compatibility layer, Shiki configuration and transformers |
| Dependencies | `comark`, `rangi`, and `@nuxt/kit` are direct runtime dependencies. Nuxt and Vue are peers. | Runtime utility, glob, Git, database, schema, Markdown, and rendering packages |
| Integrations | Sitemap entries read by `@nuxtjs/sitemap` through the collection manifest. Nuxt UI prose and content components. `content:file:beforeParse` and `content:file:afterParse` for Nuxt SEO modules. Hook input may contain transient Markdown during ingestion. Cloudflare presets require `@harlan-zw/nuxt-cloudflare` with Workers Caching enabled. | Persisted raw Markdown, Nuxt Content compatibility endpoints, database hooks, KV, and D1 |
| Build-time reads | `@harlan-zw/comark-content/server` exposes `queryCollection`, `queryCollectionNavigation`, `queryCollectionItemSurroundings`, `queryCollectionSearchSections`, `queryCollectionManifest`, and `renderPageMarkdown`. Every call runs in process against the generated assets. | HTTP calls from server code, an SSR render to read a page, a persisted raw Markdown copy |
